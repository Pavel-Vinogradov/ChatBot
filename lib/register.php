...
class Bot
{
     ...
     */
    public static function buildHandlerBackupUrl(): string
    {
    ...
    {
        return Rest:: $result = restCommand('imbot.register', array(

            'CODE' => 'newbot', // Строковой идентификатор бота, уникальный в рамках вашего приложения (обяз.)
            'TYPE' => 'B', // Тип бота, B - чат-бот, ответы поступают сразу, H - человек, ответы поступают с
            // задержкой от 2-х до 10 секунд, O - чат-бот для Открытых линий, S - чат-бот с повышенными привилегиями (supervisor)
            'EVENT_MESSAGE_ADD'     =>  Bot::buildHandlerBackupUrl(),
            // Ссылка на обработчик события отправки сообщения боту (обяз.)
            'EVENT_WELCOME_MESSAGE' =>  Bot::buildHandlerBackupUrl(),
            // Ссылка на обработчик события открытия диалога с ботом или приглашения его в групповой чат (обяз.)
            'EVENT_BOT_DELETE'      =>  Bot::buildHandlerBackupUrl() ,
     ...
     * @param array $auth - current user auth params.
     * @link https://dev.1c-bitrix.ru/learning/course/index.php?COURSE_ID=93&LESSON_ID=7893&LESSON_PATH=7657.7871.7877.7893#imbot_command_register
     */
    public static function registrationCommand($botId, $commandParams, $auth)
    {
        $commandParams['BOT_ID'] = $botId;

        return Rest::restCommand(
